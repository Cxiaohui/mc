...
class Projectoffer extends Common{
    ...
    public function info($p_id){
        ...
        if(!empty($list)){
            ...
            foreach($list as $k=>$v){
                $list[$k]['doc_count'] = $this->MD->get_count(['p_id'=>$p_id,'p_offer_id'=>$v['id'],'isdel'=>0]);
                $list[$k]['modifys'] = $projectreportmodify->get_list(['p_id'=>$p_id,'p_offer_id'=>$v['id'],'isdel'=>0],'id,type,content,addtime',0);
            }
    ...
    public function add($p_id,$id=0){
        ...
        if($id>0){
            ...
            $info['docs'] = $this->MD->get_order_list(['p_offer_id'=>$id,'isdel'=>0],'*',['seq'=>'asc'],0);
        }
    ...
    protected function add_offer_data($p_id,$id=0){
        ...
        if(!empty($docs)){
            ...
            $max_seq = $this->MD->get_max(['p_id'=>$p_id,'p_offer_id'=>$id,'isdel'=>0],'seq');
            $inserts = [];
