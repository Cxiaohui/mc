...
class Notice extends Common
{
    ...
    public function list_get(){
        ...
        $next_url = '';
        if($limit['has_next']){
            $next_url = $this->get_base_url().'/api/c_v1/notice/list?page='.($page+1).'&pagesize='.$pagesize;
        }
        $w['status'] = 0;
    ...
    public function info_get(){
        ...
        $info = $this->M->get_info(['id'=>$id,'user_type'=>2,'user_id'=>$this->user_id]);
        if(!$info){
    ...
    public function set_done_post(){
        ...
        $w = ['id'=>$id,'user_type'=>2,'user_id'=>$this->user_id];
